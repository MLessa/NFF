	...
	public class InteractionController : Controller
	{
		...
		public BaseResponse<List<InteractionDTO>> GetInteractions(string token, int count = 5, int lastID = 0)
		{
			try
			{
				...
				if (user != null)
				{
					return new BaseResponse<List<InteractionDTO>>() { Data = InteractionService.FindByFilter(null, user.UserID, count, lastID).Select(x => new InteractionDTO() { Post = new Post() { PostID = x.PostID }, User = new User() { UserID = x.UserID }, InteractionType = x.IsHot ? InteractionType.HOT : x.IsReport ? InteractionType.REPORT : InteractionType.COMMENT }).ToList(), Message = "OK", Success = true };
				}
				else
				{
